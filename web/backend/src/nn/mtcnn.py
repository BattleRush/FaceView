...
try:
    ...
    print("Failed to load: pytrt MTCNN will not work")
...
class TrtPNet(object):
    ...
    """
    input_h_offsets  = (0, 324, 554, 716, 832, 914, 972, 1014, 1044)
    output_h_offsets = (0, 162, 277, 358, 416, 457, 468, 507, 522)
    max_n_scales = 9
    ...
    def detect(self, img, minsize=40, factor=0.709, threshold=0.7):
        ...
        im_data = np.zeros((1, 3, 1064, 756), dtype=np.float32)
...
class TrtMtcnn(object):
    ...
    def __init__(self):
        #try: 
        main_path = path = os.path.dirname(__main__.__file__)

        folder = Path(main_path)

        engineName = "det1.engine"
        file_name = folder / "mtcnn" / engineName

        # TODO fix filename
        file_name = "/home/karlo/web/backend/src/mtcnn/det1.engine"

        print(file_name)

        self.pnet = TrtPNet(str(file_name))
        
        engineName = "det2.engine"
        file_name = folder / "mtcnn" / engineName

        print(file_name)
        
        # TODO fix filename
        file_name = "/home/karlo/web/backend/src/mtcnn/det2.engine"

        self.rnet = TrtRNet(str(file_name))

        
        engineName = "det3.engine"
        file_name = folder / "mtcnn" / engineName

        print(file_name)

        # TODO fix filename
        file_name = "/home/karlo/web/backend/src/mtcnn/det3.engine"

        self.onet = TrtONet(str(file_name))

        print("Loaded MTCNN TensortRT models successfully")
        #except:
        #   print("Failed to load MTCNN TensortRT models")
    ...
    def _detect_2520x1080(self, img, minsize):
        """_detect_2520x1080()

        Assuming 'img' has been resized to less than 2520x1080.
        """
